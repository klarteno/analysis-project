...
instance Show IdentifierName where
  show (IdentifierName value) =
    " " `intercalate` ["IdentifierName", show value]
...
data Statement =  AssLit Label LiteralExpr AExpr
                | AssRecord Label IdentifierName AExpr AExpr
                | IfElse BExpr Statement Statement
                | If BExpr Statement
                | While BExpr Statement
                | Skip Label
                | Read Label LiteralExpr
                | Write Label AExpr
...
data BExpr = BoolConst Label Bool
            | Not Label BExpr
            | BBinary Label BBinOp BExpr BExpr
            | RBinary Label RBinOp AExpr AExpr
            deriving (Eq,Ord,Show)
-- Expressions
...
flatListDeclarations []        = DeclEmpty
...
aTerm :: Parser AExpr
aTerm = choice
  [parens aOpExpression, try pAArray, try pARecord, try pAVariable, pAInteger]
...
-- parseTest (rExpression <* eof) "f*2 > 7"
bTerm :: Parser BExpr
bTerm = choice
...
--  parseTest (bOpExpression <* eof) "true && false||(4<7)"
...
-- parseTest ( spaceConsumer *> whileParser <* eof) "y:= x; if (3<4) {y:= x ;} else { z:= z*2;};if (3<4) {n:= n*9;}"

statementParser :: Parser Statement
statementParser = between space eof statement
...
-- parseTest ( spaceConsumer *> statementParser <* eof) "y:= x; if (3<4) {y:= x ;} else { z:= z*2;}; while (1<2) {z:= z*2;}"
...
parseString str = case parse statementParser "" str of
  Left  e -> error $ show e
  Right r -> r

parseProg :: String -> Program
parseProg str = case parse whileParser "" str of
  Left  e -> error $ show e
  Right r -> r
--  parseProg "y:= x; if (true && false) {y:= x }"
 -- parseProg "y:= x; if (3<4) {y:= x ;} else { z:= z*2;};if (3<4) {n:= n*9;}"

parseFile :: String -> IO Statement
parseFile file = do
  program <- readFile file
  case parse statementParser "" program of
